...
spinBtn.addEventListener('click', function() {
    // Kiểm tra xem các hàm Firebase đã tải xong chưa
    if (typeof window.dbRef !== 'function') {
        return alert("Hệ thống đang khởi động, vui lòng đợi vài giây!");
    }

    const name = nameInput.value.trim().toUpperCase();
    if (!name) {
        alert("Vui lòng nhập tên Ingame chính xác!");
        return;
    }
    ...
    // Check trên Firebase xem tên này đã quay chưa
    ...
    window.dbOnValue(historyRef, (snapshot) => {
        ...
        if (data) {
            // Duyệt danh sách để kiểm tra tên trùng
            hasPlayed = Object.values(data).some(item => 
                item.name && item.name.toUpperCase() === name
            );
        }

        if (hasPlayed) {
            alert("Nhân vật này đã tham gia quay rồi! Hẹn bạn tuần sau.");
            spinBtn.disabled = false; // Mở lại nút nếu bị từ chối
        } else {
            // Nếu chưa quay thì bắt đầu chạy hiệu ứng quay
            startSpinAction(name);
        }
    }, { onlyOnce: true }); // Quan trọng: chỉ chạy 1 lần duy nhất để check
});
    ...
    function animate(now) {
        ...
        if (progress < 1) {
            ...
            window.dbPush(historyRef, {
                name: name, // Phải dùng biến 'name' đã lấy ở trên
                prize: resultValue,
